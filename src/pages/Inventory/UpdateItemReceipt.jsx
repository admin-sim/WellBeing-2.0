...
import { urlCreatePurchaseOrder, urlShowBatchDetails, urlEditItemReceipt, urlAddNewItemReceipt } from '../../../endpoints.js';
...
const UpdateItemReceipt = () => {
    ...
    useEffect(() => {
        ...
        if (issueId > 0 || indentReceiptId > 0) {
            customAxios.get(`${urlEditItemReceipt}?IssueId=${issueId}&IndentReceiptId=${indentReceiptId}`).then((response) => {
                const apiData = response.data.data;
                const dummy = apiData.IndentIssueModel.IndentId != null ? apiData.newIndentIssueModel : apiData.IndentDetails
                const IndentReceiptLineId = apiData.IndentReceiptList.filter(item => apiData.newIndentIssueModel.some(item1 => item.ProductId == item1.ProductId));
                const products = dummy.map(
                    (item, index) => ({
                        ...item,
                        key: index + 1,
                        IndentReceiptLineId: IndentReceiptLineId.length > 0 ? IndentReceiptLineId[0].IndentReceiptLineId : 0
                    })
                );
                setIstablevisible(true)
                setProductcount(dummy.length)
                setData(products);
                ...
                form1.setFieldsValue({
                    ...
                    IndentReceiptId: apiData.newIndentReceiptModel != null ? apiData.newIndentReceiptModel.IndentReceiptId : 0
                });
    ...
    const columns = [
        {
            ...
            render: (_, record) => (
                <>
                    ...
                    <Form.Item name={[record.key, 'IndentReceiptLineId']} initialValue={record.IndentReceiptLineId} hidden>
                        <Input></Input>
                    </Form.Item>
        ...
        },
        {
            ...
            width: 150,
            render: (text, record) => (
                <Form.Item name={[record.key, 'AvlIssueQuantity']} initialValue={record.AvlIssueQuantity == null ? 0 : record.AvlIssueQuantity}>
                    <InputNumber min={0} disabled style={{ width: '100%' }} />
                </Form.Item>
            )
        },
        {
            ...
            width: 150,
            render: (text, record) => (
                <Form.Item name={[record.key, 'AvlReqQuantity']} initialValue={record.AvlReqQuantity == null ? 0 : record.AvlReqQuantity}>
                    <InputNumber min={0} disabled style={{ width: '100%' }} />
                </Form.Item>
            )
        },
        ...
        },
        {
            ...
            width: 150,
            render: (text, record) => (
                <Form.Item name={[record.key, 'PendingQty']} initialValue={record.PendingQty == null ? 0 : record.PendingQty}>
                    <Input style={{ width: '100%' }} disabled></Input>
                </Form.Item>
            )
        },
    ...
    const Batchmodal = [
        {
            ...
            render: (_, record) => (
                <>
                    ...
                    >
                        <Select allowClear onChange={(record) => BatchSelect(record)} style={{ width: 100 }} defaultValue={record.BatchNo}>
        ...
        },
        {
            ...
            width: 100,
            render: (text, record) => (
                <Form.Item name={[record.key, 'ReceiptRate']} initialValue={record.ReceiptRate}>
                    <Input style={{ width: '100%' }} disabled></Input>
                </Form.Item>
            )
        },
    ...
    const handleOnFinish = async (values) => {
        ...
        for (let i = 0; i <= productCount; i++) {
            if (values[i] !== undefined) {
                const product = {
                    ...
                    IndentReceiptLineId: values[i].IndentReceiptLineId ? values[i].IndentReceiptLineId : 0
                }
        ...
        if (dataModel.length > 0) {
            const Indent = {
                ...
                IndentReceiptId: values.IndentReceiptId == undefined ? 0 : values.IndentReceiptId,
            }
            const postData = {
            ...
            try {
                if (issueId > 0) {
                    const response = await customAxios.post(urlAddNewItemReceipt, postData, {
                        headers: {
                            'Content-Type': 'application/json'
                        }
                    });
