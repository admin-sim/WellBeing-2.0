...
const CreateOpeningStock = () => {
  ...
  const onOkModal = async () => {
    ...
    const newdatamodal = dataModal.map((item, index) => {
      if (index === item.key) {
        return {
          ...item,
          BarCode: form3data[index].BarCode,
          BatchNo: form3data[index].BatchNo,
          EXPDateString: form3data[index].EXPDateString ? form3data[index].EXPDateString.format("DD-MM-YYYY") : "",
          GrnBatchId: form3data[index].GrnBatchId,
          MFGDateString: form3data[index].MFGDate ? form3data[index].MFGDate.format("DD-MM-YYYY") : "",
          MRP: form3data[index].MRP,
          ProductId: form3data[index].ProductId,
          Quantity: form3data[index].Quantity,
          StockLocator: 0,
          Rate: form3data[index].PoRate,
          UomId: form3data[index].UomId,
          TaxAmount1: form3data[index].TaxAmount1 ? form3data[index].TaxAmount1 : 0,
          TaxAmount2: form3data[index].TaxAmount2 ? form3data[index].TaxAmount2 : 0,
          TaxType1: 0,
          TaxType2: 0
        }
      }
  ...
  const handleInputChange = (value, fieldName, index, record) => {
    ...
    for (let i = 0; i < counter; i++) {
      if (formdata[i].ReceivedQty != "" && formdata[i].PoRate != "") {
        formdata[i].Value = formdata[i].PoRate * formdata[i].ReceivedQty
        form2.setFieldsValue({ [record.key]: { LineAmount: formdata[i].LineAmount } })
      }
    }
      ...
      render: (text, record) => (
        <Form.Item
          ...
          rules={[
            ...
            {
              validator: (_, value) => {
                if (value < record.PoRate) {
                  return Promise.reject(
                    new Error("MRP should not be less than PO Rate.")
                  );
                }
    ...
    <Layout style={{ zIndex: '999999999' }}>
      <div style={{ width: '100%', backgroundColor: 'white', minHeight: 'max-content', borderRadius: '10px' }}>
        {/* <Row style={{ padding: '0.5rem 2rem 0.5rem 2rem', backgroundColor: '#40A2E3', borderRadius: '10px 10px 0px 0px ' }}>
          ...
          <Col offset={6} span={2}>
            <Button icon={<LeftOutlined />} style={{ marginBottom: 0 }} onClick={handleOpeningStock}>
              Back
            </Button>
          </Col>
        </Row> */}
        ...
        <ConfigProvider
          ...
          >
            ...
            >
              ...
              <Table columns={Batchmodal} dataSource={dataModal.filter((item) => item.ActiveFlag !== false)} />
            </Form>
          </Modal>
        </ConfigProvider>
      </div>
