...
const CreateOpeningStock = () => {
  ...
  useEffect(() => {
    customAxios
      .get(urlCreatePurchaseOrder, { params: { type: "Opening Stock" } })
      .then((response) => {
          ...
          if (response.status == 200 && response.data.data != null) {
            ...
            const products = editeddata.GRNAgainstPODetails.map((item) => ({
              ...
              UOM: response.data.data.UOM.filter(
                (m) =>
                  item.ProductDetails.AlternateUoms.find(
                    (i1) => i1.AlternateUom === m.UomId
                  ) || m.UomId === item.ProductDetails.UOMPrimaryUOM
              ),
              LineAmount:
                item.ProductDetails.AlternateUoms.length > 0
                  ? (() => {
                      const matchedUom = item.ProductDetails.AlternateUoms.find(
                        (x) => x.AlternateUom === item.UomId
                      );
                      return matchedUom
                        ? item.LineAmount * matchedUom.EquivalentUOMUnits
                        : item.LineAmount;
                    })()
                  : item.LineAmount,
            }));
  ...
  const onOkModal = async () => {
    ...
    const TotalQty = form3Obj.reduce(
    ...
    if (TotalQty !== productDetails.ReceivedQty) {
      ...
      const newdatamodal = dataModal.map((item, index) => {
        if (form3data[item.key] != null) {
          return {
            ...
            EXPDateString: form3data[item.key].EXPDateString
              ? form3data[item.key].EXPDateString.format("DD-MM-YYYY")
              : "",
            GrnBatchId: form3data[item.key].GrnBatchId,
            MFGDateString: form3data[item.key].MFGDateString
              ? form3data[item.key].MFGDateString.format("DD-MM-YYYY")
              : "",
            ...
            TaxType1:
              form3data[item.key].TaxType1 != ""
                ? form3data[item.key].TaxType1
                : 0,
            TaxAmount1: form3data[item.key].TaxAmount1
              ? form3data[item.key].TaxAmount1
              : 0,
            TaxType2:
              form3data[item.key].TaxType2 != ""
                ? form3data[item.key].TaxType2
                : 0,
            TaxAmount2: form3data[item.key].TaxAmount2
              ? form3data[item.key].TaxAmount2
              : 0,
          };
        }
        return item;
      ...
      form2.setFieldsValue({
        [productDetails.key]: { TaxAmount1: taxamt1 + taxamt2 },
      });
      setData(newData);
  ...
  const handleOnFinish = async (values) => {
    ...
    if (response.status == 200) {
      message.success(
        `Stock ${GRNHeaderId == 0 ? "Created" : "Updated"} Successfully`
      );
      handleOpeningStock();
    } else {
      ...
      newData = data.map((item) => {
        if (item.key === record.key) {
          ...
          const altUom = altUomData
            ? altUomData.data.find(
                (i1) =>
                  i1.AlternateUom == updatedItem.UomId ||
                  i1.UomId == updatedItem.UomId
              )
            : undefined;
          const recievingQty =
          ...
          form2.setFieldsValue({
            [record.key]: { TaxAmount1: item.TaxAmount1 },
          });
    ...
    },
    {
      ...
      render: (text, record, index) => (
        <Form.Item
          ...
          initialValue={record.ReceivedQty}
          rules={[
            {
              required: true,
              message: "Please input!",
    ...
    },
    {
      ...
      render: (text, record) => (
        <Form.Item
          ...
          initialValue={record.Replaceable}
  ...
  const calculateTax = (amount, taxDetails, record) => {
    ...
    const altUom = altUomData
      ? altUomData.data.find((i1) => i1.AlternateUom == productDetails.UomId)
      : undefined;
  ...
  const Batchmodal = [
        ...
        <>
          ...
          >
            <Input
              min={0}
              style={{ width: 100 }}
              disabled={!!record.GrnBatchId}
            />
          </Form.Item>
          <Form.Item
            name={[record.key, "ProductId"]}
            hidden
            initialValue={productDetails.ProductId}
          >
            <Input></Input>
          </Form.Item>
          <Form.Item
    ...
    },
    {
      ...
      render: (text, record) => (
        <Form.Item
          name={[record.key, "UomId"]}
          initialValue={productDetails.UomId}
        >
    ...
    },
    {
      ...
      render: (text, record) => (
        <Form.Item
          name={[record.key, "PoRate"]}
          initialValue={productDetails.PoRate}
        >
