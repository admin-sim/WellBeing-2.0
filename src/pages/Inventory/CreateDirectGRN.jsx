...
import { urlCreatePurchaseOrder, urlAutocompleteProduct, urlGetProductDetailsById, urlAddNewGRNDirect } from '../../../endpoints';
   ...
   */
    ...
    const handleOnFinish = async (values) => {
        ...
        if (delivery.length > 0) {
            ...
            for (let i = 0; i <= idCounter; i++) {
                if (values[i] !== undefined) {
                    const product = {
                        ...
                        PoRate: values[i].poRate === undefined ? 0 : values[i].poRate.toFixed(4),
                        DiscountRate: values[i].discount === undefined ? 0 : values[i].discount.toFixed(4),
                        DiscountAmount: values[i].discountAmt === undefined ? 0 : parseFloat(values[i].discountAmt).toFixed(4),
                        // DiscountAmount: values[i].discountAmt === "" ? 0 : (form1.getFieldValue([i, 'discountAmt'])).toFixed(4),          
                        TaxAmount1: values[i].taxAmount === "" ? 0 : values[i].taxAmount.toFixed(4),
                        TotalAmount: values[i].totalAmount === "" ? 0 : values[i].totalAmount,
                        Replaceable: values[i].replaceable === undefined ? 'true' : values[i].cgstAmt.toFixed(4),
            ...
            const DirectGRN = {
                ...
                DCChallanNumber: values.DCChallanNumber === undefined ? null : values.DCChallanNumber,
                Remarks: values.Remarks === undefined ? null : values.Remarks,
                GRNStatus: values.GRNStatus === undefined ? 'Created' : values.GRNStatus,
                InvoiceAmount: values.InvoiceAmount === undefined ? null : values.InvoiceAmount,
                Amount: values.Amount === undefined ? null : values.Amount,
                TotalPoAmount: values.totalpoAmount === undefined ? null : values.totalpoAmount,
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (text, record) => (
                <Form.Item name={[record.key, 'uom']}>
                    <Select value={selectedUom[record.key]} style={{ width: '100%' }} onChange={(option) => handleSelectChange(record.key, option)}>
        ...
        },
        {
            ...
            key: 'recievingQty',
            render: (text, record) => (
                <Form.Item name={[record.key, 'recievingQty']}
                    rules={[
                        {
                            required: true,
                            message: 'Please input!'
        ...
        },
        {
            ...
            key: 'poRate',
            render: (text, record) => (
                <Form.Item name={[record.key, 'poRate']} initialValue={text}
                    rules={[
                        {
                            required: true,
                        },
        ...
        },
        {
            ...
            width: 100,
            key: 'Batch',
            render: (value, record) => <Button type="link" onClick={() => ModelOpen(value, record)}>Batch</Button>
        },
        {
            title: 'Amount',
            dataIndex: 'amount',
    ...
    const columnsModel = [
        {
            ...
            key: 'barcode',
            render: (_, record) => (
                <Form.Item name={[record.key, 'barcode']} style={{ width: '100%' }}>
                    <InputNumber min={0} />
                </Form.Item>
            )
        },
        ...
        },
        {
            ...
            key: 'quantity',
            render: (text, record) => (
                <Form.Item name={[record.key, 'quantity']} style={{ width: '150%' }}
                    rules={[
                        {
                            required: true,
                            message: "input!"
        ...
        },
        {
            ...
            key: 'bonusqty',
            render: (text, record) => (
                <Form.Item name={[record.key, 'bonusqty']} initialValue={form1.getFieldValue([productLineId, 'BonusQty'])} style={{ width: '150%' }}>
        ...
        },
        {
            ...
            render: (text, record) => {
                ...
                <Form.Item name={[record.key, 'muom']} initialValue={selectedUomId[recordKeyAsNumber]}>
        ...
        <Layout style={{ zIndex: '999999999' }}>
            <div style={{ width: '100%', backgroundColor: 'white', minHeight: 'max-content', borderRadius: '10px' }}>
                <Row style={{ padding: '0.5rem 2rem 0.5rem 2rem', backgroundColor: '#40A2E3', borderRadius: '10px 10px 0px 0px ' }}>
                    ...
                    <Col offset={6} span={2}>
                        <Button icon={<LeftOutlined />} style={{ marginBottom: 0 }} onClick={handleToDirectGRN}>
                            Back
                        </Button>
                    </Col>
                </Row>
                    ...
                    onValuesChange={(changedValues, allValues) => {
                        ...
                        for (let j = 0; j < 10; j++) {
                            if (allValues[j] !== undefined) {
                                if (allValues[j]['recievingQty'] > 0 && allValues[j]['poRate'] > 0) {
                ...
                >
                    <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }} style={{ padding: '1rem 2rem', marginBottom: '0' }} align="Bottom">
                        ...
                        <Col className="gutter-row" span={6}>
                            <div>
                                <Form.Item
                                    label="DC Challan Number"
                                    name="DCChallanNumber"
                                >
                ...
                <ConfigProvider
                    ...
                    >
                        ...
                        >
                            <Col className="gutter-row" span={6}>
                                <Form.Item label="Product" name="Product" initialValue={selectedProductId[recordKeys]}>
                                    <Tag color="blue">{form1.getFieldValue([recordKeys, 'product'])}</Tag>
                                </Form.Item>
                            </Col>
                            <Col className="gutter-row" span={6}>
                                <Form.Item label="Recieved Quantity" name="RecievedQty" initialValue={selectedProductId[recordKeys]}>
                                    <Tag color="blue">{form1.getFieldValue([recordKeys, 'recievingQty'])}</Tag>
                                </Form.Item>
                            </Col>
                            <Col className="gutter-row" span={6}>
                                <Form.Item label="Bonus Quantity" name="BonusQuantity" initialValue={selectedProductId[recordKeys]}>
                                    <Tag color="blue">{form1.getFieldValue([recordKeys, 'bounsQty'])}</Tag>
                                </Form.Item>
                            </Col>
