...
function BillAggrementModal({
  ...
  const onFinishForAddChargeParameters = async (values) => {
    debugger;
    setLoading(true);
    values.AgreementId = agreementId;
    values.AgreementLineId = 0;
    values.RevisionNo = 0;
    ...
    try {
      ...
      if (response.status == 200 && response.data) {
        if (response.status === 200 && response.data.data != null) {
          ...
          handleCancel();
          message.success("BillAgreement Parameter Added Successfully...");
        } else {
          message.error("BillAgreement With Same Name Already Exists");
        }
      }
    } catch (error) {
  ...
  return (
    <div>
      <Spin spinning={loading}>
        <Modal
          title="Bill Agreement Line Details"
          open={open}
          maskClosable={false}
          footer={null}
...
export default BillAggrementModal;
