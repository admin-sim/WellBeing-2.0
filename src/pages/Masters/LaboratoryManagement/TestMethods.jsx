...
import { urlTestMethodIndex, urlLoadTestMethodGridData, urlSaveTestMethod, urlDeleteTestMethod } from "../../../../endpoints";
...
const TestMethods = () => {
        ...
        if (!hasEffectRun.current) {
            try {
                customAxios.get(urlTestMethodIndex).then((response) => {
                    ...
                    const MainOptions = apiData.AllDiagnosticTests.map(item => ({ value: item.ServiceId, label: item.LongName }));
                    // const SubOptions = apiData.MainTests.map(item => ({ value: item.ServiceId, label: item.LongName }));
                    // setSubTestMappingIndex(apiData);
                    setSubTestMappingIndex((prevState) => ({
                        ...prevState,
                        AllDiagnosticTests: [...prevState.AllDiagnosticTests, ...MainOptions],
                        // SubTests: [...prevState.SubTests, ...SubOptions]
                    }));
                });
            } catch (error) {
    ...
    const onChange = (value) => {
        debugger
        try {
            customAxios.get(`${urlLoadTestMethodGridData}?TestId=${value}`).then((response) => {
                debugger;
    ...
    const handleDelete = (row) => {
        debugger;
        try {
            customAxios.delete(`${urlDeleteTestMethod}?testmethodId=${row.TestMethodID}`).then((response) => {
                debugger;
                ...
                <Card>
                    ...
                    >
                        <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
                            ...
                            <Col className="gutter-row" span={8}>
                                <Form.Item name="MethodsName" label="Method Name"
                                    rules={[
                                        {
                                            required: true,
                                            message: 'Please input!'
