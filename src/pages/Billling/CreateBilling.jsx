...
import { urlGetAllAutocompleteServicesAsync, urlAddNewBill, urlGetServiceCharge, urlGetAllProviders, urlAddNewCharge, urlBillingCreate, urlGetPatientDetails } from '../../../endpoints';
...
const CreateBilling = () => {
  ...
  const totalAmount = charges?.reduce((total, row) => total + row.PatientNetAmount, 0) ?? 0;
  ...
  const handleSelect = async (value, option) => {
    ...
    if (option.key) {
      try {
        ...
        if (newData[0]) {
          form.setFieldsValue({ Amount: newData[0].ChargeAmount, PatientAmount: newData[0].PatientNetAmount,
  ...
  const columns = [
    {
      title: <span style={{ display: 'flex', justifyContent: 'flex-start' }}>ChargeDetails</span>,
    ...
    {
      title: <span style={{ display: 'flex', justifyContent: 'flex-start' }}>PatientAmount</span>,
      children: [
        {
          title: 'Charge',
        ...
        },
        {
          ...
          render: (_, row) => (
            <span style={{ display: 'flex' }}>
              <Tooltip title="Edit">
                <Button type='link' onClick={() => handleEdit(row)} ><EditOutlined style={{ fontSize: ".8rem" }} /></Button>
              </Tooltip>
              <Tooltip title="Delete">
                <Button type='danger' onClick={() => handleEdit(row)} ><DeleteOutlined style={{ fontSize: ".8rem" }} /></Button>
              </Tooltip>
            </span>
          ),
        },
  ...
  const receiptInscolumns = [
      ...
      render: (text, record, index) => (
        <Form.Item
          ...
          rules={[
            ...
            {
              validator: (_, value) => {
                const otherRows = receiptInsAmtData.filter((row) => row.key !== record.key);
                const duplicateExists = otherRows.some((row) => row.PaymentTypeId === value);
                if (duplicateExists) {
                  // return Promise.reject('Payment Type already selected in another row');
                  return Promise.reject(new Error('Payment Type should not be same'));
                }
        ...
        >
          <Select onChange={value => handleInputChange(value, 'PaymentTypeId', record.key)}>
      ...
      render: (text, record) => (
        <Form.Item
          ...
          rules={[
            { required: true, message: 'Required' },
              {
              validator: (_, value) => {
                const newTotal = totalInstrumentAmount - (parseFloat(record.InstrumentAmount) || 0) + value;
                if (newTotal > totalAmount) {
                  return Promise.reject(new Error('Total Amount should not be greater than ReceiptAmount'));
                }
                return Promise.resolve();
              }
            }
        ...
        >
          <InputNumber
            style={{ width: "100%" }}
            min={0}
            onChange={value => handleInputChange(value, 'InstrumentAmount', record.key)}
          />
        </Form.Item>
      ),
    },
    {
      ...
      render: (text, record, index) => (
        <Form.Item
          name={['BankId', record.key - 1]}
        >
          <Select onChange={value => handleInputChange(value, 'BankId', record.key)}>
            {banks?.map((option) => (
              <Option key={option.LookupID} value={option.LookupID}>
                {option.LookupDescription}
              </Option>
    ...
    },
    {
      ...
      render: (text, record, index) => (
        ...
        >
          <Input min={0} defaultValue={text} onChange={value => handleInputChange(value  , 'BranchName',  record.key)} />
        </Form.Item>
      ),
    },
    {
      ...
      render: (text, record, index) => (
        ...
        >
          <Input min={0} defaultValue={text} onChange={value => handleInputChange(value, 'IFSC', record)} />
        </Form.Item>
      ),
    },
    {
      ...
      render: (text, record, index) => (
        ...
        >
          <Input min={0} defaultValue={text} onChange={value => handleInputChange(value, 'AuthorizationReference', record)} />
        </Form.Item>
      ),
    },
  ...
  const handleInstrumentDelete = (record) => {
    ...
    setReceiptInsAmtData(receiptInsAmtData.filter(item => item.key !== record.key));
  ...
  const handleOnFinish = async (values) => {
    ...
    const Charge = {
      ...
      ChargeAmount: values.PatientAmount,
      FacilityId: 1,
      ActiveFlag: true,
      ServiceQuantity: 1,
      PatientTypeID: 22
  ...
  const handleSaveBill = async (values) => {
    ...
    const formattedReceiptInsAmtData = receiptInsAmtData.map(item => ({
      ...
      IFSC: item.IFSC || '',
      InstrumentAmount: item.InstrumentAmount ? parseFloat(item.InstrumentAmount) : 0,
    ...
    try {
      ...
      if (response.status === 200 && response.data) {
        if (response.data === 'Failed To Generate Bill' || response.data === '') {
  ...
  return (
    <Layout style={{ zIndex: '999999999' }}>
      <div style={{ width: '100%', backgroundColor: 'white', minHeight: 'max-content', borderRadius: '10px' }}>
        ...
        <Row style={{ padding: '0.2rem 2rem 0rem 2rem', backgroundColor: '#40A2E3', borderRadius: '5px 5px 0px 0px ' }}>
          <Col span={16}>
              ...
              <div>
                ...
                >
                  <AutoComplete
                    ...
                    onChange={(value) => {
                      if (!value) {
                        form.setFieldsValue({ Amount: "", PatientAmount: "",Provider:"" });
                        setServices(null);
        ...
        >
          <Table
            ...
            size="small"
            locale={{ emptyText: <span style={{ color: '' }}>No data available</span> }}
            bordered
            pagination={{
            ...
            summary={(pageData) => {
              ...
              let patientnetamt = 0;
              let adjamt = 0;
              pageData.forEach(({ NetAmount, InsuranceCoveredAmount, TaxRate, NetInsurenceAmount, PatientDiscountAmount, PatientTaxRate, PatientNetAmount, AdjustedAmount }) => {
                netamt += NetAmount;
              ...
              return (
                <>
                  <Table.Summary.Row>
                    ...
                    <Table.Summary.Cell index={2} colSpan={9}></Table.Summary.Cell>
                  </Table.Summary.Row>
                </>
              );
            }}
