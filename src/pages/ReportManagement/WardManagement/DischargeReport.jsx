...
function DischargeReport() {
  ...
  const fetchDataHeader = async () => {
    ...
    try {
      const response = await customAxios.get(
        `${urlGetAllType}?Type=Disposition Type`
      );
      if (response.status === 200 && response.data != null) {
        const paymentdetail = response.data.data;
        setDispositionType(paymentdetail);
      } else {
      }
    } catch (error) {}
  ...
  const onFinish = async (values) => {
    ...
    if (values.DispositionType === "") {
      values.DispositionType = 0;
    }
  ...
  return (
    <Layout style={{ width: "100%" }}>
      ...
      >
        <PageHeader title={"Discharge Report"} button={false} />
        <div
          style={{
            padding: "1rem",
            borderRadius: "0.5rem",
