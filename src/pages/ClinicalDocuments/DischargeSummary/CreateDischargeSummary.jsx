...
const DischargeForm = () => {
  ...
  const handleAddRow = async () => {
    ...
    setDataSource([
      ...dataSource,
      {
        ...
        ActiveFlag: true,
        PrescriptionStatus: true,
        DischargeSummaryMedicationId: 0
      },
    ]);
    // const newData = {
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <>
          ...
          <Form.Item
            hidden
            name={[record.key, "ActiveFlag"]}
            initialValue={record.ActiveFlag}
          >
            <Input />
          </Form.Item>
          <Form.Item
          ...
          <Form.Item
            hidden
            name={[record.key, "DischargeSummaryMedicationId"]}
            initialValue={record.DischargeSummaryMedicationId}
          >
            <Input />
          </Form.Item>
        </>
  ...
  const handleSubmit = async (values) => {
    ...
    for (let i = 0; i < dataSource.length; i++) {
      const item = form2data[dataSource[i].key] || dataSource[i]; 

      if (
        item.DischargeSummaryMedicationId === dataSource[i].DischargeSummaryMedicationId
      ) {
        const Drug = {
          DischargeSummaryMedicationId: item.DischargeSummaryMedicationId ?? 0, 
          DischargeSummaryID: dsModel?.DischargeSummaryID ?? 0,
          DrugId: item.DrugId || 0, 
          UomId: item.UomId || 0, 
          DrugName: item.Drug || item.DrugName, 
          Route: item.Route ? item.Route.toString() : "", 
          FrequencyId: item.Frequency || 0, 
          Interval: item.IntervalInDays ? parseInt(item.IntervalInDays) : 0, 
          Instruction: item.Instruction || "", 
          EncounterId: Patient?.EncounterId || null, 
          PatientId: Patient?.PatientId || null,
          Dsmstatus: item.ActiveFlag,
        };
        Drugss.push(Drug);
        // } else {
        //   const Drug = {
        ...
        //     DrugName: dataSource[i].Drug || 0, // default to 0 if Dose is null or undefined
      ...
      try {
        ...
        if (response.status === 200 && response.data.data) {
          ...
          form.setFieldsValue({
            ...
            followUpDate: data[0]?.FollowUpCheck ? dayjs(data[0].FollowUpDate1, "DD-MM-YYYY") : dayjs(),
            surgeryDate: data[0]?.SurgeryCheck ? dayjs(data[0].Surgery1, "DD-MM-YYYY") : dayjs(),
          ...
          setFormData((prev) => ({
            ...prev,
            showFollowUpDate: data[0]?.FollowUpCheck
          }));

          setFormData((prev) => ({
            ...prev,
            showSurgeryDate: data[0]?.SurgeryCheck
          }));

          const medicationData = medications.map((med) => ({
            key: uuidv4(),
            DrugId: med.DrugId,
            DischargeSummaryMedicationId: med.DischargeSummaryMedicationId,
            DrugName: med.DrugName || null,
            Route: parseInt(med?.Route) || "",
            FrequencyId: med?.FrequencyId || 0,
            Interval: med?.Interval || 0,
            Instruction: med?.Instruction || "",
            ActiveFlag: med.DSMStatus
          }));
