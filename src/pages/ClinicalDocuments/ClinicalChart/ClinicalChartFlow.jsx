...
function ClinicalChartFlow() {
  ...
  const handleSelect = (value, option) => {
    ...
    try {
      customAxios
        .get(`${urlGetLastEncounter}?Uhid=${option.value}`)
        .then((response) => {
          if (response.data.data.length > 0) {
            setEncounter(response.data.data);
            form.setFieldsValue({
              EncounterId: response.data.data[0].EncounterId,
            });
            form.setFieldsValue({ PatientId: option.key });
            form.setFieldsValue({
              Encounter: response.data.data[0].GeneratedEncounterId,
            });
          } else {
            setEncounter([]);
  ...
  const columns = [
    {
      ...
      render: (text, record, index) => (
        <>
          {text}
          <Form.Item hidden name="ServiceID" initialValue={record.ServiceID}>
            <Input />
          </Form.Item>
          <Form.Item hidden name="ChargeID" initialValue={record.ChargeID}>
            <Input />
          </Form.Item>
        </>
      ),
    ...
    },
    {
      ...
      dataIndex: "ReturnQty",
      key: "ReturnQty",
      render: (text, record) => <InputNumber min={0} allowClear />,
    },
    {
    ...
    },
    {
      ...
      dataIndex: "actions",
      key: "actions",
      render: (text, record) => record.ReturnQty * record.Rate,
    },
  ];
  ...
  return (
    <Layout style={{ width: "100%" }}>
      ...
      >
        <PageHeader title={"Clinical Chart Flow"} button={false} />
            ...
            <>
              ...
              <Table
                ...
                summary={(pageData) => {
                  ...
                  let rate = 0;
                  let returnqty = 0;
                  pageData.forEach(({ ReturnQty, Rate }) => {
                    returnqty += ReturnQty;
                    rate += Rate;
                  });
                  return (
...
//             `${urlGetEncounterDetails}?PatientId=${record.PatientId
