...
const OrderDetails = (Patient) => {
  ...
  async function handleSendtoLab() {
    ...
    const investigations = Patient.dropDown.PatientAccountCharges.filter(
      (f) => f.ServiceGroupID == 1042
    );
    const listnotsentToLab = investigations.filter(
      (f) => f.SamplColHeaderId == null || f.SamplColHeaderId == 0
    );
    if (
      listnotsentToLab.length > 0 &&
      Patient.dropDown.LastEncounter.PatientType != 22
    ) {
      const BillViewModel = {
